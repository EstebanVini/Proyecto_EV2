...
def guardar_pelicula(movie: Movie, username: str):
    ...
    try:
        cursor.execute("INSERT INTO movies (title, release, type, genre, imageurl, user1, user2) VALUES (?, ?, ?, ?, ?, ?, ?)", (movie.title, movie.release, movie.type, movie.genre, movie.imageurl, user.username, user.relatedto))
        conn.commit()
        return True
...
def get_all_movies(username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE (user1 = ? OR user2 = ?)", (user.username, user.username,))
...
def get_tvseries(username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE type = 'tvSeries' AND user1 = ? OR user2 = ?", (user.username, user.username))
...
def get_movies(username: str):
    ...
    # ejecutar la consulta SQL para obtener las películas
    try:
        cursor.execute("SELECT * FROM movies WHERE (user1 = ? OR user2 = ?) AND type = 'movie'", (user.username, user.username,))
        movies = cursor.fetchall()
        return [Movie(id=movie[0], title=movie[1], release=movie[2], type=movie[3],genre=movie[4], imageurl=movie[5]) for movie in movies]
    except Exception as e:
        print(f"Error: {str(e)}")
...
def get_movie_by_id(id, username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE id = ? AND (user1 = ? OR user2 = ?)", (id, user.username, user.username))
...
def get_movie_by_genre(genre: str, username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE genre = ? AND (user1 = ? OR user2 = ?)", (genre, user.username, user.username))
        movies = cursor.fetchall()
        return [Movie(id=movie[0], title=movie[1], release=movie[2], type=movie[3],genre=movie[4], imageurl=movie[5]) for movie in movies]
    except Exception as e:
        print(f"Error: {str(e)}")
...
def get_random_movie(username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE user1 = ? OR user2 = ? ORDER BY RANDOM() LIMIT 1", (user.username, user.username))
        movie = cursor.fetchone()
        return Movie(id=movie[0], title=movie[1], release=movie[2], type=movie[3],genre=movie[4], imageurl=movie[5])
    except Exception as e:
        print(f"Error: {str(e)}")
...
def get_random_movie_by_genre(genre: str, username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE genre = ? AND (user1 = ? OR user2 = ?) ORDER BY RANDOM() LIMIT 1", (genre, user.username, user.username))
        movie = cursor.fetchone()
        return Movie(id=movie[0], title=movie[1], release=movie[2], type=movie[3],genre=movie[4], imageurl=movie[5])
    except Exception as e:
        print(f"Error: {str(e)}")
...
def get_random_tvseries(username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE user1 = ? OR user2 = ? AND type = 'tvSeries' ORDER BY RANDOM() LIMIT 1", (user.username, user.username))
...
def get_random_movie(username: str):
    ...
    # ejecutar la consulta SQL para obtener una película aleatoria
    try:
        cursor.execute("SELECT * FROM movies WHERE user1 = ? OR user2 = ? AND type = 'movie' ORDER BY RANDOM() LIMIT 1", (user.username, user.username))
        movie = cursor.fetchone()
        return Movie(id=movie[0], title=movie[1], release=movie[2], type=movie[3],genre=movie[4], imageurl=movie[5])
    except Exception as e:
        print(f"Error: {str(e)}")
...
def get_random_movie_by_genre_and_type(movie: Movie, username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE genre = ? AND type = ? AND (user1 = ? OR user2 = ?) ORDER BY RANDOM() LIMIT 1", (movie.genre, movie.type, user.username, user.username))
        movie = cursor.fetchone()
        return Movie(id=movie[0], title=movie[1], release=movie[2], type=movie[3],genre=movie[4], imageurl=movie[5])
    except Exception as e:
        print(f"Error: {str(e)}")
...
def search_movie_by_title_DB(title: str, username: str):
    ...
    try:
        cursor.execute("SELECT * FROM movies WHERE title LIKE ? AND (user1 = ? OR user2 = ?)", ('%' + title + '%', user.username, user.username))
...
def update_movie(movie: Movie, username: str):
    ...
    try:
        cursor.execute("UPDATE movies SET title = ?, release = ?, type = ?, genre = ?, imageurl = ? WHERE id = ? AND (user1 = ? OR user2 = ?)", (movie.title, movie.release, movie.type, movie.genre, movie.imageurl, movie.id, user.username, user.username))
        conn.commit()
        return True
    except Exception as e:
        print(f"Error: {str(e)}")
